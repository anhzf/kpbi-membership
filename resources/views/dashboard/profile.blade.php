...
<div class="col-span-2 p-16 pt-5 relative">
    <form action="{{ route('profile:update') }}" method="POST">
        ...
        <div class="group-input">
            ...
            <x-input.text name="alamat_kampus[kota]" label="Kota/Kabupaten" placeholder="Kota/Kabupaten" class="w-1/2" value="{{ $data->alamat_kampus['kota'] ?? '' }}"></x-input.text>
            <x-input.text name="alamat_kampus[provinsi]" label="Provinsi" placeholder="Provinsi" class="w-1/2" value="{{ $data->alamat_kampus['provinsi'] ?? '' }}"></x-input.text>
        </div>
        <div class="group-input">
            <x-input.text name="kaprodi[nama]" label="Nama Lengkap dan Gelar Kaprodi" placeholder="Nama Lengkap dan Gelar Kaprodi" class="w-full" value="{{ $data->kaprodi['nama'] ?? '' }}"></x-input.text>
            <x-input.text name="kaprodi[periode][mulai]" label="Periode Jabatan Kaprodi" placeholder="(tahun mulai)" class="w-2/4" value="{{ $data->kaprodi['periode']['mulai'] ?? '' }}"></x-input.text>
            <x-input.text name="kaprodi[periode][akhir]" label="Purna" placeholder="(tahun purna)" class="w-1/4" value="{{ $data->kaprodi['periode']['akhir'] ?? '' }}"></x-input.text>
            <x-input.text name="kaprodi[email]" type="email" label="Email Kaprodi" placeholder="Email Kaprodi" class="w-full" value="{{ $data->kaprodi['email'] ?? '' }}"></x-input.text>
            <x-input.text name="kaprodi[no]" type="tel" label="Nomor HP Kaprodi" placeholder="Nomor HP Kaprodi" class="w-full" value="{{ $data->kaprodi['no'] ?? '' }}"></x-input.text>
        </div>
        
        <x-input.text name="email_prodi" type="email" label="Email Prodi" placeholder="Alamat email Prodi" value="{{ $data->email_prodi ?? '' }}"></x-input.text>
        <x-input.text name="no_telp_prodi" type="tel" label="Nomor Telpon Kantor" placeholder="Nomor Telpon Kantor" value="{{ $data->no_telp_prodi ?? '' }}"></x-input.text>
